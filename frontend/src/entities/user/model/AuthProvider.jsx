...
import {
  MOCK_USERS,
  getMockUserByEmail } from
"../../../shared/mocks/mockUsers.js";
...
export const AuthProvider = ({ children }) => {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        ...
        if (accessToken && accessToken.startsWith("mock_token_")) {
          ...
          if (existingData) {
            console.log(
              "Returning cached mock user data:",
              existingData
            );
            return existingData;
          }
    ...
    onError: (err) => {
      console.error(
        "ОШИБКА useQuery('me'): Не удалось загрузить профиль. Выход из системы.",
        err
      );
      setAccessToken(null);
    }
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!csrfToken) {
          console.log(
            "[Auth] CSRF-токен не найден, прекращаем попытку восстановления."
          );
          return;
        }

...
export default function ProductDetail({ orgId, productId, product: mockProduct, onAdd, onBack, onChat, isSupplierView = false }) {
  ...
  const { data: apiProduct, isLoading, isError, error } = useQuery({
    ...
    enabled: !!orgId && !!productId, // Только если есть orgId (значит пользователь из настоящего API)
    retry: false,
  });

  // Используем API данные, если они есть, иначе fallback на мок данные
  const product = apiProduct || mockProduct;

  const [quantity, setQuantity] = useState(1);
  const [selectedImage] = useState(0); // setSelectedImage не используется, убираем его
  ...
  // Адаптируем данные для совместимости с API и мок данными
  const productTitle = product.brand || product.title || 'Товар';
  const productPartNumber = product.part_number || '';
  const productImage = product.media?.[0]?.url || product.img || 'https://via.placeholder.com/400';
  
  const images = [
    productImage,
    productImage, // В реальности здесь будут разные изображения
    productImage,
    productImage
  ];
	...
	<div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-8">
		<div className="max-w-7xl mx-auto px-4">
			<button 
				...
				onClick={() => {
					console.log("Back button clicked in ProductDetail");
					onBack && onBack();
				}}
			>

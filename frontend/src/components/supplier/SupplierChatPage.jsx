...
/**
 * SupplierChatPage - компонент чата специально для поставщика
 * Отображает чаты с покупателями и заказами
 */
export default function SupplierChatPage() {
  ...
  const loadSupplierChats = () => {
    // Simulate loading time and filter chats for supplier view
    setTimeout(() => {
      // Фильтруем чаты для поставщика - показываем только чаты с покупателями
      const supplierChats = MOCK_CHATS.map(chat => ({
        ...chat,
        // Для поставщика показываем имя покупателя как основное
        title: chat.buyerName || chat.title,
        subtitle: `Заказ #${chat.orderId || Math.floor(Math.random() * 10000)}`,
        role: "supplier" // помечаем как чат поставщика
      }));
  ...
  const sendMessage = (content) => {
    ...
    setTimeout(() => {
      ...
      // Симулируем ответ покупателя через некоторое время
      setTimeout(() => {
        const buyerResponse = {
